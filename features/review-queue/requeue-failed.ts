...
type FailedAnalysisRow = { id: string; pull_request_id: string | null };
...
export type RequeueResult = { requeued: number; skipped: number };

/**
 * Requeue every analysis that failed in the last 24 hours. Shared by the
 * machine route (/api/internal/requeue-failed, cron/CRON_SECRET auth) and the
 * admin server action behind the /internal panel. Auth is the caller's
 * responsibility — this function does the work only.
 */
export async function requeueFailedAnalyses(): Promise<RequeueResult> {
  ...
  for (const analysis of failed) {
    ...
    // Clear any stale ownership claim before re-enqueuing, else the worker
    // sees the claim, backs off, and the row stays stuck at 'queued'.
